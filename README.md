...
1. Copy `.env.example` to `.env` and fill in your Shopify credentials (store
   name, Admin API token, API key and secret).
2. Install Python dependencies with `pip install -r requirements.txt`.
3. Run `python deploy_shopify.py` to connect the domain defined in
   `APP_DOMAIN` and mark it as primary.

The helper script uses your credentials from `.env` and the Shopify Admin API to
add the domain if it does not already exist. Ensure the DNS records are set
before running it.
