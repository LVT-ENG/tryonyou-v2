...
- `scripts/auto_update.py` keeps datasets, new IP drops and OpenAI integrations in sync. Use `--interval N` to run on a schedule so the platform always stays updated.
- Import the flows in `automa_flows/` into the Automa browser extension. The `site_health_email.automa.json` flow works with `scripts/site_health_check.py` to send uptime alerts and ensure the app keeps producing.
