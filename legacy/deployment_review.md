...
- `public/404.html` provides a custom 404 page for unmatched routes.
- `meet_pau.html` and `pau_emotion.html` now include description and canonical tags for basic SEO compliance.

## 2. API server
- `server.js` exposes routes such as `/tryonme-core` and `/autodonate/check` that rely on the OpenAI API and are used by the front-end.
- The server also serves static files from the `public/` directory for production deployment.
