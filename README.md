...
- Run `npm run build` to generate the static files in the `dist/` folder. This is the same command used by Vercel and Netlify during deployment.
...
- Para configurar el entorno Codex en Netlify, revisa [docs/netlify_codex_setup.md](docs/netlify_codex_setup.md).
...
Run `npm run build` to generate the static `dist/` directory used by Vercel and Netlify. This copies the main HTML, CSS and assets so the site can be deployed anywhere.
...
For Netlify deployments with Codex environment, the system automatically runs `scripts/prepare_codex_env.sh` before building. See [docs/netlify_codex_setup.md](docs/netlify_codex_setup.md) for details.
