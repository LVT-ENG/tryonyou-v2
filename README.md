...
Run `python scripts/generate_image_meta.py` to create `products/image_meta.json` with alt text and social image tags for each product.
...
## 🛡️ Error Handling
The server now logs uncaught exceptions in `system_errors.log` to reduce
unexpected stops. See [docs/error_handling.md](docs/error_handling.md) for
details.
