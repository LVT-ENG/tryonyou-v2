...
### Automation script
You can automate these steps with `scripts/shopify_domain_connect.js`. Set the environment variables `SHOPIFY_EMAIL` and `SHOPIFY_PASSWORD` and run:

```bash
node scripts/shopify_domain_connect.js
```
The script logs the DNS records to configure and waits for Shopify to finish verifying the domain.
