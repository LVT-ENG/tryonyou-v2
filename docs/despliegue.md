...
Para automatizar el despliegue y registrar en Notion puedes usar `deploy_and_log.js`:

```bash
node scripts/deploy_and_log.js          # despliega y registra en Notion
node scripts/deploy_and_log.js log      # solo registra en Notion
```

Si se pasa cualquier otro argumento, el script mostrará un mensaje de error.
