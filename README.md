...
## 🚀 Shopify Auto Deploy
The repository includes a GitHub Actions workflow at `.github/workflows/shopify-deploy.yml` that automatically pushes the theme to Shopify whenever you push to the `main` branch. Add `SHOPIFY_STORE_DOMAIN` and `SHOPIFY_ACCESS_TOKEN` as repository secrets and the action will run `shopify theme push` for you.
