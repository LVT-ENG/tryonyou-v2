...
Use `python scripts/sync_to_shopify.py` to upload products defined in `products/tryon-products.json` (or `products/casa_pavo_real.json` as a fallback) directly to your Shopify store. The Python script expects `SHOPIFY_STORE_DOMAIN` and `SHOPIFY_ACCESS_TOKEN` variables. A Node.js variant is available via `node scripts/upload_to_shopify.js` which reads `SHOPIFY_STORE` and `SHOPIFY_TOKEN`.
