...
Set `OPENAI_API_KEY`, `SITE_URL`, `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS` and `NOTIFY_EMAIL` in the environment before deploying. The server and health‑check script handle missing values gracefully, but some features will be disabled without them.
