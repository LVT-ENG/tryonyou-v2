...
## 🧩 Codex Automation Snippets

This repo includes handy snippets for automating common tasks:

### Netlify Deploy Agent

Run the `codexDeployAgent` module to push the contents of `tryonme-final-deploy` to Netlify:

```javascript
import codexDeployAgent from './agents/codexDeployAgent.js';
codexDeployAgent.deploy();
```
...
Embed a Shopify product on any page:
...
Upload products to Shopify from the terminal:

```bash
curl -X POST -u admin:API_KEY \
  -H 'Content-Type: text/csv' \
  --data-binary @productos.csv \
  https://tryonyou.myshopify.com/admin/products.json
```
