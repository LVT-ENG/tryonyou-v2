...
5. Verify the domain is active:
   ...
   ```
   You should see `configured: true` once DNS propagation is complete.

Once DNS propagation completes, the message will disappear and your site will load from the custom domain.

To automate this process in CI, set the `VERCEL_DOMAIN` environment variable. Deployment scripts will call `vercel domains add $VERCEL_DOMAIN` before deploying so your domain is always configured.
